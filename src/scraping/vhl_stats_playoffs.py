...
output_excel_path = results_dir / f"Results_VHL_{SEASON}_Playoffs.xlsx"
...
def game_no_from_url(u: str, season: str = SEASON) -> int:
    m = re.search(rf"/VHL{season}-(\d+)\.html", u)
...
def fetch_and_parse_page(
    ...
    url = (
        f"https://vhlportal.com/vhl/{SEASON}/Playoffs/"
        f"VHL{SEASON}-PLF-{game_no}.html"
    )
    try:
...
def build_names_from_games_and_enrich_pos() -> (
    ...
    def fetch_one(team_id: int):
        team_url = f"https://vhlportal.com/vhl/{SEASON}/ProTeam.php?Team={team_id}"
        try:
...
def scrape_all_roster_attributes() -> (
    ...
    for team_id in range(1, TEAM_PAGE_MAX_ID + 1):
        url = f"https://vhlportal.com/vhl/{SEASON}/ProTeam.php?Team={team_id}"
        try:
